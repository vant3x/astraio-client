use super::{ContentType, CookieSnapshot, HttpRequestView, Message, ResponseTab, ScriptTab};
...
impl HttpRequestView {
    ...
    fn create_cookies_tab_content(&self) -> Element<'_, Message, Theme, iced::Renderer> {
        use crate::ui::views::cookie_manager::CookieManagerView;
    ...
    fn render_cookie_table(
        &self,
        manager: &crate::ui::views::cookie_manager::CookieManagerView,
        ...
        container(
            column![
                row![
                    iced::widget::text_input("Search cookies...", &manager.search_query)
    ...
    fn render_cookie_badges(&self, cookie: &CookieSnapshot) -> Element<'_, Message, Theme, iced::Renderer> {
        ...
        if cookie.secure {
            badges = badges.push(cookie_badge("S", BadgeKind::Secure));
        }
        if cookie.http_only {
            badges = badges.push(cookie_badge("H", BadgeKind::HttpOnly));
        }
        match cookie.same_site.as_str() {
            "Strict" => {
                badges = badges.push(cookie_badge("St", BadgeKind::SameSiteStrict));
            }
            "None" => {
                badges = badges.push(cookie_badge("N", BadgeKind::SameSiteNone));
            }
            _ => {
                badges = badges.push(cookie_badge("L", BadgeKind::SameSiteLax));
            }
        }
...
fn cookie_badge<'a>(
    icon: &'a str,
    kind: crate::ui::views::cookie_manager::BadgeKind,
